...
const globalRef = globalThis as typeof globalThis & {
  ...
  __crmSeedPromise__?: Promise<void>;
};
...
async function seedCrmIfNeeded(): Promise<void> {
  globalRef.__crmSeedPromise__ ??= (async () => {
    const sql = await getSql();
    const { ensureCrmSeeded } = await import("@/lib/crm/seed");
    await ensureCrmSeeded(sql);
  })().catch((err) => {
    globalRef.__crmSeedPromise__ = undefined;
    console.error("[db] CRM seed failed:", err);
    throw err;
  });
  await globalRef.__crmSeedPromise__;
}
...
export function ensureDbReady(): Promise<void> {
  if (dbSource !== "pglite") {
    // On Neon, seed still runs lazily on first getSql path via server functions.
    return Promise.resolve();
  }
  return getSql()
    .then(() => seedCrmIfNeeded())
    .then(() => undefined);
}
